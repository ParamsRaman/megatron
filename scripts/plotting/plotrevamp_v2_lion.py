    ...
    with open(filename, 'r') as f:
        for line in f:
            ...
            if ' lr_warmup_fraction ' in line:
                ...
                print("w_frac: ", res['hyperparams']['warmup_fraction'])
            ...
            if ' weight_decay ' in line:
                res['hyperparams']['weight_decay'] = line.strip().split(' ')[2]
...
def make_samplesbasedplot(plot_title, job_ids, rampup_batchsize=False, grad_norm=False, xlog=False, ylog=False, log_tp_size=False, lookahead=False, lion=False):
    plt.figure()
    for job in job_ids:
        plt_data = data[job]
        ...
        if rampup_batchsize:
            ...
            legendlabel = plt_data['hyperparams']['optimizer']  + \
                ...
                ' lr=' + plt_data['hyperparams']['lr'] + \
                ' warmup=' + warmup + \
                ' wd=' + plt_data['hyperparams']['weight_decay'] + \
                ' samples=' + plt_data['hyperparams']['train_samples']
            if log_tp_size:
...
if __name__ == "__main__":
    ...
    if TR_LMLOSS_SAMPLES:
        make_samplesbasedplot(plot_fname, job_ids, RAMPUP_BATCHSIZE, GRAD_NORM, XLOG, YLOG, LOG_TP_SIZE, LOOKAHEAD, LION)
